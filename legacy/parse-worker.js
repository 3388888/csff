...
// native decoder — writes the gzipped raw JSON straight to the cache file. Same interface
// whether it's the Go csgofast or the Rust csgo-rs (built with --features app).
function runNativeDecoder(demPath, exe, outGz, onProgress) {
  return new Promise((resolve, reject) => {
    ...
    ch.on("error", reject);
    ch.on("close", (code) => code === 0 ? resolve() : reject(new Error("decoder failed (exit " + code + "): " + err.split("\n").filter(Boolean).pop())));
  });
}

// Prefer the Rust decoder (csgo-rs), fall back to the Go one (csgofast). Rust exits non-zero
// on non-CS:GO demos, which is exactly the signal to try the Go path (or drop to CS:S below).
function runCsgofast(demPath, msgOrExe, outGz, onProgress) {
  // back-compat: callers may pass the msg object (has .csgors/.csgofast) or a bare exe path
  if (typeof msgOrExe === "string") return runNativeDecoder(demPath, msgOrExe, outGz, onProgress);
  const { csgors, csgofast } = msgOrExe;
  const tryGo = () => csgofast && fs.existsSync(csgofast)
    ? runNativeDecoder(demPath, csgofast, outGz, onProgress)
    : Promise.reject(new Error("no decoder available"));
  if (csgors && fs.existsSync(csgors)) {
    return runNativeDecoder(demPath, csgors, outGz, onProgress).catch(tryGo);
  }
  return tryGo();
}
...
process.on("message", async (msg) => {
  ...
  try {
    ...
    if (!raw) {
      ...
      const haveNative = (msg.csgors && fs.existsSync(msg.csgors)) || (msg.csgofast && fs.existsSync(msg.csgofast));
      if (haveNative && msg.rawWrite && isFile) {
        try {
          await runCsgofast(msg.path, msg, msg.rawWrite, onProgress);
      ...
      if (!usedNative) {
        ...
        if (((msg.csgors && fs.existsSync(msg.csgors)) || (msg.csgofast && fs.existsSync(msg.csgofast))) && msg.rawWrite) {
          try { await runCsgofast(p, msg, msg.rawWrite, onProgress); raw = JSON.parse(zlib.gunzipSync(fs.readFileSync(msg.rawWrite))); raw.demPath = p; usedNative = true; } catch { raw = null; }
        }
        if (!usedNative) {
