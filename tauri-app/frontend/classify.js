// Browser bundle: the REAL classify + cssff rulebook, wrapped in a CommonJS shim so the
// source is UNMODIFIED (no fragile stripping). Node-only bits (fs/demofile) are stubbed and
// never called (parseRaw is unused here).
(function () {
  function makeRequire(cssffcfgMod) {
    return function (name) {
      if (name === "fs") return { readFile(){}, readFileSync(){ throw new Error("no fs"); }, statSync(){ throw new Error("no fs"); } };
      if (name === "demofile") return { DemoFile: function(){} };
      if (name === "./cssffcfg") return cssffcfgMod.exports;
      return {};
    };
  }
  var cssffMod = { exports: {} };
...
module.exports = { load, parse, rules, val, num, bool };
  ...
  })(cssffMod, cssffMod.exports, makeRequire(cssffMod));

  var parserMod = { exports: {} };
  (function (module, exports, require) {
/* ==== parser.js ==== */
/*
...
module.exports = { parseRaw, classify, parseDemo, tickHints, DEFAULTS, TAGW };
  ...
  })(parserMod, parserMod.exports, makeRequire(cssffMod));

  var classify = parserMod.exports.classify;
  var cssffcfg = cssffMod.exports;
  window.classifyRaw = function (raw, iniText, opts) {
    opts = Object.assign({}, opts || {});
    if (iniText && iniText.trim()) opts.cssff = Object.assign({ ok: true }, cssffcfg.parse(iniText));
    return classify(raw, opts);
  };
})();
